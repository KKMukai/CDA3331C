...
//that is, no action if  (1.1 * lightroom < light < 1.5 * lightroom).
        ...
        if(light < lightroom * 1.00 && light > lightroom * 1.001)
          {}
        else
          {
            if(light >= lightroom * 1.00) {P1OUT |=  BIT4; __delay_cycles(200);} // on if dark
            if(light <= lightroom * 1.001) {P1OUT &= ~BIT4; __delay_cycles(200);} // off if light
          }
...
//Temperature Controlling LED2
        ...
        if(temp > temproom * 1.01 && temp < temproom * 1.03)
          {;}
