...
for (;;)
{
    ...
    if (((switches & BIT5) == BIT5) && ((switches & BIT4) != BIT4) && ((switches & BIT3) == BIT3))
    {
           leftdigit = pleftdigit;     //left is now present
           rightdigit = prightdigit;   //right is now present
    }
    ...
    // *** modify this section according to the lab manual requirement
    {
        if(leftdigit == 9)           //checks if 9
        {
            leftdigit = pleftdigit; //left and right assigned to present value
            rightdigit = prightdigit;
        }
        else
        {
            if(rightdigit < 9)      //less than 9
                rightdigit++;       //increment
            else
            {
                leftdigit++;        //increment
                rightdigit = 0;    //start over
            }
        }
    ...
    // *** modify this section according to the lab manual requirement
    {
        if((leftdigit == 1 && rightdigit == 0) || (leftdigit == 0)) //10 or 0
        {
            leftdigit = pleftdigit;     //left is now present
            rightdigit = prightdigit;   //right is now present
        }
        ...
        {
            if(rightdigit != 0)  //not 0
                rightdigit--;   //decrement
            else
            {
                rightdigit = 9; //decrease 10's
                leftdigit--;
            }
        }
    }
