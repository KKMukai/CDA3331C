...
Summation	mov.w 	R5, R6		;starting the upper limit with value a of the summation in R6
			...
			add.w R6, R6			;finish Xcalcby dobling the value and adding it to accumulator R8,
			add.w R6, R8			;repeat the loop with a-1, a-2 (in R5) till lower value a=0
			dec.w R5				;save the Xcalc results in R5
			cmp.w #0, R5
			jge Summation		;loop
			mov.w R8, R5		;move result from R8 to R5

Fcalc		mov.w 	R5, R7		;start F calculation

			and.w #7, R5			;multiply by 7
			add.w #25, R5			;finish Fcalc by adding the constant 25 and dividing by 8 (hint shift twice to the right with carry=0)
			rrc R5
			rrc R5
			rrc R5


loop		jmp 	loop		;loop in place, infinite loop
...
FACTO:
			...
			push R5		;use the MULT subroutine to multiply R5 to R6 registers
			clr R6			;clear
			cmp.w #0, R5	;checks if n=0
			jne next		;keep checking
			add.w #1, R6	;set = 1
			jmp FACTOend

next		cmp.w #1, R5	;checks if n=1
			jeq int
			jge next2
			mov.w #0, R6	;n=0
			jmp FACTOend	;done

int			mov.w  #1, R6	;checks if n=1
			jmp FACTOend

next2		mov.w R5, R6	;save to R6
...
;end of you're code between double dashed lines
;-------------------------------------------------------------------------------
;-------------------------------------------------------------------------------
